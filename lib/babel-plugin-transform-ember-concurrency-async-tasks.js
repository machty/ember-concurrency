...
const TransformAsyncMethodsIntoGeneratorMethods = {
  ...
  ClassProperty(path) {
    ...
    // path.traverse(MaybeTaskProperty);
    // if (hasTaskDecorator(path)) {
    // } else if (isTaskOrTaskGroupFactoryFunction(path)) {
    // }
  },
};
...
const TaskProperty = {
  CallExpression(path) {
    // ember-concurrency-async: check for `@task x = taskFor(...)`
    if (hasTaskDecorator(path) && isTaskFor(path.get('callee'))) {
