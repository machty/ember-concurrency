...
module("Unit: Restartable policy", function() {
  test("maxConcurrency 1 cancels the earliest running instance", function(assert) {
    ...
    assert.deepEqual(typesFor(policy, 1, 2), ["CANCELLED", "CANCELLED", "STARTED"]);
    assert.deepEqual(typesFor(policy, 1, 3), ["CANCELLED", "CANCELLED", "CANCELLED", "STARTED"]);
  });
  ...
  test("maxConcurrency 2 keeps the first two running", function(assert) {
    ...
    assert.deepEqual(typesFor(policy, 1, 2), ["CANCELLED", "STARTED", "STARTED"]);
    assert.deepEqual(typesFor(policy, 1, 3), ["CANCELLED", "CANCELLED", "STARTED", "STARTED"]);
  });
});
