...
module("Unit: Drop policy", function() {
  test("maxConcurrency 1 cancels the earliest running instance", function(assert) {
    ...
    assert.deepEqual(typesFor(policy, 1, 2), ["STARTED", "CANCELLED", "CANCELLED"]);
    assert.deepEqual(typesFor(policy, 1, 3), ["STARTED", "CANCELLED", "CANCELLED", "CANCELLED"]);
  });
  ...
  test("maxConcurrency 2 keeps the first two running", function(assert) {
    ...
    assert.deepEqual(typesFor(policy, 1, 2), ["STARTED", "STARTED", "CANCELLED"]);
    assert.deepEqual(typesFor(policy, 1, 3), ["STARTED", "STARTED", "CANCELLED", "CANCELLED"]);
  });
});
